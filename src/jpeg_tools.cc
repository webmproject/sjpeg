...
static SjpegYUVMode RiskinessImpl(const uint8_t* rgb,
  ...
  const auto ScoreRow = [&]() {
    ...
    for (; i + 8 <= width - 1; i += 8) {
      const uint16x8_t r1 = vld1q_u16(&row1[i]);
      const uint16x8_t r1_shift = vld1q_u16(&row1[i + 1]);
      const uint16x8_t r2 = vld1q_u16(&row2[i]);
      ...
      vst1q_u16(p0, r1);
      vst1q_u16(p1, r1_shift);
      vst1q_u16(p2, r2);

      for (int k = 0; k < 8; ++k) {
        const int k2 = kRGB3 * p2[k];
        const int score = sjpeg::kSharpnessScore[p0[k] + kRGB3 * p1[k]]
                        + sjpeg::kSharpnessScore[p0[k] + k2]
                        + sjpeg::kSharpnessScore[p1[k] + k2];
    ...
    for (; i < width - 1; ++i) {
      ...
      const int k2 = kRGB3 * idx2;
      const int score = sjpeg::kSharpnessScore[idx0 + kRGB3 * idx1]
                      + sjpeg::kSharpnessScore[idx0 + k2]
                      + sjpeg::kSharpnessScore[idx1 + k2];
      if (score > kNoiseLevel) {
        score_sum += score;
        score_num += 1;
      }
